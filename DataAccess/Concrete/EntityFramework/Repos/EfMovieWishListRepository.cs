...
{
    public class EfMovieWishListRepository : EfRepositoryBase<MovieWishList, LibraryContext>, IMovieWishListRepository
    {
        public EfMovieWishListRepository(LibraryContext context) : base(context)
        {
        }

        public async Task<List<MovieResponseModel>> GetByAccount(int id)
        {
            var query = from a in Context.Accounts.Include(a => a.MovieWishLists).ThenInclude(l => l.Movie).Where(a => a.Id == id)
                        select a.MovieWishLists.Select(l => l.Movie);
