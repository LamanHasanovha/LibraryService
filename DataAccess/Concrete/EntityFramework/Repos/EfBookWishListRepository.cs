...
{
    public class EfBookWishListRepository : EfRepositoryBase<BookWishList, LibraryContext>, IBookWishListRepository
    {
        public EfBookWishListRepository(LibraryContext context) : base(context)
        {
        }

        public async Task<List<BookResponseModel>> GetByAccount(int id)
        {
            var query = from a in Context.Accounts.Include(a => a.BookWishLists).ThenInclude(l => l.Book).Where(a => a.Id == id)
                        select a.BookWishLists.Select(l => l.Book);
